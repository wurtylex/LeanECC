...
\begin{lemma}[subspace-imp-generator]
    ...
    \uses{def:GeneratorMatrix}
    Let $C$ be a linear code that is a subspace of $\mathbb{F}^{n}_{q}$ per LC_subspace, then $\exists G$ an $kxn$ matrix such that $C=\{fG | f\in \mathbb{F}^{k}_{q}\}$
\end{lemma}
...
\begin{lemma}[subspace-imp-parity-check]
    \uses{def:LC_subspace}
    \uses{def:ParityCheckMatrix}
    \uses{lemma:subspace-imp-generator}
    Let $C$ be a linear code that is a subspace of $\mathbb{F}^{n}_{q}$ per LC_subspace, then $\exists H$ an $n-kxn$ matrix such that $C=\{f | f\in \mathbb{F}^{n}_{q}, Hf^t=0\}$
\end{lemma}
