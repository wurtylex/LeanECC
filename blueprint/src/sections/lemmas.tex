...
\begin{lemma}[subspace-imp-parity-check]
    ...
    Let $C$ be a linear code that is a subspace of $\mathbb{F}^{n}_{q}$ per subspace-def, then $\exists H$ an $n-kxn$ matrix such that $C=\{f | f\in \mathbb{F}^{n}_{q}, Hf^t=0\}$
\end{lemma}

\begin{proof}
    Suppose we take the dual space of $C$, $C^*$, maybe using (https://leanprover-community.github.io/mathlib4_docs/Mathlib/LinearAlgebra/Dual/Basis.html)
    We can then take the basis vectors of $C^*$ and create an $H$ with them as rows.
\end{proof}

\begin{lemma}[parity-check-imp-subspace]
    \uses{def:subspace-def}
    \uses{def:parity-check-def}
    Let $H$ an $n-kxn$ over $\mathbb{F}_{q}$ matrix such that $C=\{f | f\in \mathbb{F}^{n}_{q}, Hf^T=0\}$, then $C$ is a subspace of $\mathbb{F}^{n}_{q}$
\end{lemma}
