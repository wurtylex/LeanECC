...
\begin{definition}[Maximal Wth Respect to Inclusion]
  \label{def:Vol}
  \lean{Code.maximalWrtInclusion}
  \leanok
  \uses{def:HammingBall}
  \uses{def:Code, def:q, def:dim, def:minDist}
  Let $C$ be a code over alphabet $q$ with block length $n$ and minimum distance $d$.
  $C$ is defined to be maximal with respect to inclusion if:
  $C'$ being an code over alphabet $q$ with block length $n$ and minimum distance $d$ $\implies$ $\lvert C'\rvert\leq \lvert C\rvert$
\end{definition}
